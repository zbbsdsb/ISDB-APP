...
jest.mock('../../hooks/use-toast', () => ({
  useToast: () => ({show: jest.fn(), ToastComponent: null}),
}));
...
jest.mock('../../constants/m3-shape', () => ({
  m3Shape: {small: 8, medium: 12, large: 16},
}));
...
describe('SwipeScreen', () => {
  ...
  it('renders Discover title', () => {
    ...
    expect(
      tree.root.find(
        (el: any) => el.type === 'Text' && el.props.children === 'Discover',
      ),
    ).toBeTruthy();
  });
  ...
  it('shows loading state', () => {
    ...
    expect(
      tree.root.find(
        (el: any) =>
          el.type === 'Text' && el.props.children === 'Loading projects...',
      ),
    ).toBeTruthy();
  });
  ...
  it('shows error state with retry button', () => {
    ...
    expect(
      tree.root.find(
        (el: any) =>
          el.type === 'Text' && el.props.children === 'Something went wrong',
      ),
    ).toBeTruthy();
    expect(
      tree.root.find(
        (el: any) => el.type === 'Text' && el.props.children === 'Retry',
      ),
    ).toBeTruthy();
  });
  ...
  it('shows empty state when no projects', () => {
    ...
    expect(
      tree.root.find(
        (el: any) =>
          el.type === 'Text' &&
          el.props.children === 'No more projects to discover!',
      ),
    ).toBeTruthy();
    expect(
      tree.root.find(
        (el: any) => el.type === 'Text' && el.props.children === 'Refresh',
      ),
    ).toBeTruthy();
  });
    ...
    mockState.projects = [
      {
        ...
        tags: ['react'],
        required_skills: [],
        owner: {username: 'u1', display_name: 'User 1'},
      },
    ];
    const tree = renderer.create(<SwipeScreen />);
    expect(
      tree.root.find(
        (el: any) => el.type === 'Text' && el.props.children === 'Test Project',
      ),
    ).toBeTruthy();
  });
  ...
  it('shows Undo button when canUndo is true', () => {
    ...
    expect(
      tree.root.find(
        (el: any) => el.type === 'Text' && el.props.children === 'Undo',
      ),
    ).toBeTruthy();
  });
  ...
  it('handles missing owner display_name gracefully', () => {
    ...
    expect(
      tree.root.find(
        (el: any) => el.type === 'Text' && el.props.children === 'No Name',
      ),
    ).toBeTruthy();
  });
});
