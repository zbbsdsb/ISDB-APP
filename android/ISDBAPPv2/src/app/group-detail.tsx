...
export function GroupDetailScreen() {
  ...
  const { fetchGroupDetail, fetchGroupMembers, joinGroup, leaveGroup, loading } = useGroups();
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.outlineVariant }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.onBackground }]}>Group</Text>
        <View style={{ width: 48 }} />
      </View>

      <ScrollView contentContainerStyle={styles.scroll}>
        {/* Name + Description */}
        <Text style={[styles.groupName, { color: colors.onBackground }]}>{group.name}</Text>
        {group.description && (
          <Text style={[styles.desc, { color: colors.onSurfaceVariant }]}>{group.description}</Text>
        )}

        {/* Stats */}
        <View style={styles.statsRow}>
          <Card variant="filled" padding={m3Spacing.md} style={styles.statCard}>
            <Text style={[styles.statNumber, { color: colors.primary }]}>{group.member_count}</Text>
          ...
          <Card variant="filled" padding={m3Spacing.md} style={styles.statCard}>
            ...
            <Text style={[styles.statLabel, { color: colors.onSurfaceVariant }]}>Type</Text>
          </Card>
        </View>
        ...
        <View style={styles.tabRow}>
          <Button title="Info" onPress={() => setActiveTab('info')}
            variant={activeTab === 'info' ? 'filled' : 'text'} size="sm" />
          <Button title={`Members (${members.length})`} onPress={() => setActiveTab('members')}
            variant={activeTab === 'members' ? 'filled' : 'text'} size="sm" />
        </View>

        {activeTab === 'info' && (
          <View style={styles.tabContent}>
            <Card variant="outlined" padding={m3Spacing.md}>
              <Text style={[styles.infoLabel, { color: colors.onSurfaceVariant }]}>Created</Text>
              <Text style={[styles.infoValue, { color: colors.onSurface }]}>
                {new Date(group.created_at).toLocaleDateString()}
              </Text>
              {group.owner && (
                <>
                  <Text style={[styles.infoLabel, { color: colors.onSurfaceVariant, marginTop: m3Spacing.sm }]}>Owner</Text>
                  <Text style={[styles.infoValue, { color: colors.onSurface }]}>
        ...
        {activeTab === 'members' && (
          <View style={styles.tabContent}>
            {members.map((member) => (
              <Card key={member.id} variant="outlined" padding={m3Spacing.md} style={styles.memberCard}>
                <View style={styles.memberRow}>
                  <View style={[styles.memberAvatar, { backgroundColor: colors.secondaryContainer }]}>
                    <Text style={[styles.memberAvatarText, { color: colors.onSecondaryContainer }]}>
                      {member.user_id.charAt(0).toUpperCase()}
                    </Text>
                  </View>
                  <View>
                    <Text style={[styles.memberRole, { color: colors.onSurface }]}>
                      {member.role}
                    </Text>
                    <Text style={[styles.memberId, { color: colors.onSurfaceVariant }]}>
...
const styles = StyleSheet.create({
  ...
  tabRow: { flexDirection: 'row', gap: m3Spacing.xs, marginVertical: m3Spacing.lg },
  tabContent: { gap: m3Spacing.sm },
  infoLabel: { ...m3Typography.labelMedium, marginBottom: 2 },
  infoValue: { ...m3Typography.bodyMedium },
  memberCard: {},
  memberRow: { flexDirection: 'row', alignItems: 'center' },
  memberAvatar: { width: 36, height: 36, borderRadius: 18, justifyContent: 'center', alignItems: 'center', marginRight: m3Spacing.sm },
