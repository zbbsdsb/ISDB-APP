...
interface MatchItem {
  ...
  other_user?: { display_name?: string; username?: string; identity_number?: number };
}
...
export function MatchesScreen() {
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <Button title="Start Swiping" onPress={() => navigation.navigate('Swipe')} style={{ marginTop: 16 }} />
    </View>
  );
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.header}>
        <Text style={[styles.title, { color: colors.onBackground }]}>Matches</Text>
      </View>
      <FlatList
        data={matches}
        ...
        renderItem={({ item }) => (
          <Card
            ...
            onPress={() => navigation.getParent()?.navigate('ProjectDetail', { projectId: item.project_id })}
          >
            <Text style={[styles.matchTitle, { color: colors.onSurface }]}>
              {item.project?.title || 'Project'}
            </Text>
            <Text style={[styles.matchStatus, { color: colors.onSurfaceVariant }]}>
              {item.status === 'pending' ? 'Pending Approval' : item.status}
            </Text>
          </Card>
        )}
        ListEmptyComponent={loading ? null : renderEmptyState}
        contentContainerStyle={[styles.list, matches.length === 0 && styles.emptyList]}
        refreshing={loading}
      />
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  matchStatus: { ...m3Typography.bodySmall, marginTop: 2, textTransform: 'capitalize' },
