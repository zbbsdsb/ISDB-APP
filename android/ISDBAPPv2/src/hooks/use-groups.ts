...
interface UseGroupsResult {
  ...
  createGroup: (data: { name: string; description?: string; tags?: string[] }) => Promise<string | null>;
...
export function useGroups(): UseGroupsResult {
  ...
  const { setGroups, setJoinedGroupIds, joinGroup: storeJoin, leaveGroup: storeLeave } = useGroupStore();
  ...
  const fetchGroupDetail = useCallback(async (groupId: string): Promise<GroupWithDetails | null> => {
    try {
      const { data, error: fetchError } = await supabase
        .from('groups')
        .select('*, owner:profiles!owner_id(id, username, display_name, avatar_url, identity_number)')
        .eq('id', groupId)
        .single();
  ...
  const getUserRole = async (groupId: string): Promise<'owner' | 'admin' | 'member' | null> => {
    if (!user) return null;
    const { data } = await supabase
      .from('group_members')
      .select('role')
