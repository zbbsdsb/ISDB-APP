...
export function useSwipe(): UseSwipeResult {
  ...
  const loadProjects = useCallback(async (pageNum = 0) => {
    if (!user) return;
    ...
    try {
      ...
      let query = supabase
        .from('projects')
        .select('id, title, description, tags, required_skills, github_url, cover_image_url, card_color, hook_text, owner:profiles!owner_id(username, display_name, avatar_url)')
      ...
      const { data, error: fetchError } = await query.range(rangeStart, rangeEnd);
  ...
  }, [currentIndex, projects.length, hasMore, loadingMore, loading, page, loadProjects]);
