...
export default function RoadmapBlock({ config }: RoadmapBlockProps) {
  ...
  return (
    <Card variant="elevated" padding={m3Spacing.md} style={styles.card}>
      ...
      {milestones.length === 0 ? (
        <Text style={[styles.emptyText, { color: colors.onSurfaceVariant }]}>No milestones yet</Text>
      ) : (
        milestones.map((ms, idx) => (
          <View key={ms.id || idx} style={styles.milestoneRow}>
            <View style={[styles.statusDot, { backgroundColor: STATUS_COLORS[ms.status] || '#94a3b8' }]} />
            <View style={styles.milestoneContent}>
              <Text style={[styles.milestoneTitle, { color: colors.onBackground }]}>{ms.title}</Text>
              {ms.description && (
                <Text style={[styles.milestoneDesc, { color: colors.onSurfaceVariant }]}>{ms.description}</Text>
              )}
              <View style={styles.metaRow}>
                <View style={[styles.statusBadge, { backgroundColor: (STATUS_COLORS[ms.status] || '#94a3b8') + '20' }]}>
                  <Text style={[styles.statusText, { color: STATUS_COLORS[ms.status] || '#94a3b8' }]}>
...
const styles = StyleSheet.create({
  ...
  statusDot: { width: 10, height: 10, borderRadius: 5, marginTop: 6, marginRight: m3Spacing.sm },
  ...
  statusBadge: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: m3Shape.small },
