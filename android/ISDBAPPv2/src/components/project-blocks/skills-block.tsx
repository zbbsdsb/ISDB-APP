    ...
    <Card variant="elevated" padding={m3Spacing.md} style={styles.card}>
      ...
      {required.length > 0 && (
        <View style={styles.subSection}>
          <Text style={[styles.subTitle, { color: colors.onSurfaceVariant }]}>Required</Text>
      ...
      {preferred.length > 0 && (
        <View style={styles.subSection}>
          <Text style={[styles.subTitle, { color: colors.onSurfaceVariant }]}>Preferred</Text>
          <View style={styles.chipRow}>
            {preferred.map((s) => (
              <View key={s} style={[styles.chip, { backgroundColor: colors.tertiaryContainer }]}>
                <Text style={[styles.chipText, { color: colors.onTertiaryContainer }]}>{s}</Text>
              </View>
            ))}
          </View>
        </View>
      ...
      {required.length === 0 && preferred.length === 0 && (
        <Text style={[styles.emptyText, { color: colors.onSurfaceVariant }]}>No skills specified</Text>
      )}
    </Card>
  );
}
...
const styles = StyleSheet.create({
  ...
  chip: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: m3Shape.small },
